...
def add_operation(operatoin):
    ...
    if value[-1] in '+-*/':
        ...
        value = calc.get()
    calc.delete(0, tk.END)
    calc.insert(0, value+operatoin)

def calculate():
    value = calc.get()
    if value[-1] in "-+*/":
        value = value+value[:-1]      # Операция с числом на само себя по умолчанию
    calc.delete(0, tk.END)
    try:
        calc.insert(0, eval(value))
    except (NameError, SyntaxError):
        messagebox.showinfo('Внимание', 'Вы ввели другие символы. Необходимо вводить только цифры')
...
def make_digit_button(digit):
    return tk.Button(text=digit, bd=5, font=('Arial', 10), command=lambda: add_digit(digit))
...
def press_key(evant):
    print(evant.char)
    if evant.char.isdigit():
        add_digit(evant.char)
    elif evant.char in '+-*/' :
        add_operation(evant.char)
    elif evant.char == '\r' :
        calculate()
...
window["bg"] = 'gray1'  # Цвет фона(таблица ткинтер)
window.bind('<Key>', press_key)

calc = tk.Entry(window, justify=tk.RIGHT, font=('Arial', 15), width=15)
calc.insert(0,'0')
calc.grid(row=0, column=0, columnspan=4, stick='we', padx=5)  # Поле ввода
